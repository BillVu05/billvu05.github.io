...
* Implemented AI in the marketing team's live business workflows, connecting **Claude** to **n8n**, **HubSpot**, and campaign channels so LLM-powered automations replaced manual steps in day-to-day CRM, content, and advertising operations
* Built an AI-assisted **CRM data-entry automation**, using Claude Code to write the integration that inputs and formats practitioner, client, contract, and personal information into HubSpot, processing **500+ contact records** and cutting data-formatting error rates by **50%**
* Worked hands-on on **Health Hunter**, a healthcare marketing platform, establishing practitioner and patient-client records in **HubSpot CRM** and keeping them aligned so ongoing client management ran off consistent data
* Analysed practitioner marketing and information pages on Health Hunter, using **paid ads**, **SEO**, and performance data analysis to maximise impressions for practitioners reaching clients and patients seeking care
* Shipped **4 end-to-end LLM content pipelines** in n8n (SEO blog expander, video-to-content-pack, blog-to-social repurposing, ad copy refresh), calling the Claude API from webhook-triggered workflows to eliminate manual production steps across recurring campaign cycles
...
* Engineered an email subject-line **A/B testing** pipeline and a weekly analytics summary emailer, automating recurring campaign testing and reporting workflows
* Scoped automation initiatives with business stakeholders, running requirements sessions and translating business intent into BRDs, process flow documentation, and implementation specs for handover
* Measured the impact of automated campaigns across **GA4**, **Google Search Console**, **Google Merchant Center**, and **Shopify**, building **Looker Studio** reports and advanced Excel models (**Power Query**, pivot tables, XLOOKUP, what-if analysis, Solver) that fed findings back into SEO and paid campaign optimisation
...
* Designed and built ETL/ELT pipelines in Python and SQL loading **1,200+ HR, onboarding, and operational records** into PostgreSQL, on **daily incremental scheduled loads** with schema validation and deduplication
...
* Engineered an ETL pipeline ingesting real BITRE international aviation statistics, World Bank GDP/population/tourism indicators, OurAirports coordinates, and real competitor schedules/fares/ratings (Singapore Airlines, Qantas, ANA, Air NZ, Jetstar, Virgin Australia); recalibrated the market-share logit model and fare formulas against real benchmarks, e.g. Singapore Airlines' reported ~60% Australia-Singapore share, enforced by a regression test holding the modelled share in a realistic band
* Built a **model-selection pipeline for demand forecasting** that scores a seasonal-index model and **XGBoost** against two naive baselines on a forward-looking 2025 holdout, deploying the winner only if it beats the best baseline by a set margin: the deployed seasonal-index model reached **MAPE 4.37% (R²=0.997)**, ahead of same-month-last-year (4.70%) and XGBoost (9.27%)
* Replaced a leaky shuffled k-fold with **leave-one-route-out cross-validation** and built prediction intervals from prospective rolling-origin out-of-fold residuals; designed a **confidence-scoring system** combining 200-resample bootstrap spread, per-route relative forecast reliability, and uncapped extrapolation distance into one explainable 0–100 score, reported in bands across the forecasting, simulation, and reporting UI
...
* Built a **conversational AI executive chat agent** (`/chat`) backed by Gemini with 12 function-calling tools: route simulation, demand trend forecasting, macro projection, multi-year P&L analysis, Monte Carlo risk analysis, network portfolio ranking, and full strategic feasibility analysis for any airport worldwide
* Developed **multi-year macro projection models** (GDP via EWMA trend + IMF long-run mean reversion, population via OLS extrapolation, tourism via pre-COVID structural CAGR, fuel price via a discrete Ornstein-Uhlenbeck mean-reversion model) feeding an **open route analyst** that scores strategic feasibility for any of 159 worldwide airports: market sizing from a **gravity model fitted on 36 real Sydney city pairs** (BITRE 2024, leave-one-out backtested), multi-factor risk scoring, composite 0–100 scoring, and side-by-side ranking of up to 8 candidate destinations
* Designed a **PostgreSQL schema** with ETL load scripts and a Docker Compose deployment (FastAPI backend, Next.js frontend, and an opt-in PostgreSQL container for the ETL), covered by **142 regression tests**
* Built a **7-page Next.js dashboard** (App Router, TypeScript, Tailwind CSS, Recharts, React-Leaflet) spanning route exploration, market/demand/revenue intelligence, AI agent orchestration, a conversational copilot, risk stress-testing, multi-year forecasting, and a persisted strategic report library, using Google's AI-powered **Stitch** design tool (via its MCP server) to design and sync the entire visual system to React components across every page
...
**Python, LangGraph, Google Gemini (generation + embeddings), NumPy, SerpAPI, Docker, Pytest, Ruff, GitHub Actions | github.com/BillVu05/llm-research-agent | Jul 2025**

* Built a CLI research agent turning a natural-language question into a cited answer as strict JSON, implemented as a LangGraph `StateGraph` over six nodes (generate_queries, web_search, ingest, retrieve, reflect, synthesize) with a typed state schema
* Implemented a conditional edge out of `reflect`: when retrieved context fails to cover a required fact, the graph routes back to `web_search` with model-written follow-up queries, bounded by a round budget so a persistent gap terminates instead of looping; the required "slots" are derived per question by the existing query-planning call rather than a hardcoded topic table, at no extra LLM cost
* Built real RAG in place of snippet-answering: `ingest` fetches top result pages concurrently, extracts text with a stdlib `HTMLParser` that drops nav/script/footer chrome, and splits it into overlapping chunks so a fact straddling a boundary stays retrievable; fetching is best-effort, so a 403, paywall, PDF, or timeout falls back to the search snippet rather than an empty context
* Built the retrieval stage as lexical prefilter, batched embeddings, cosine similarity, MMR, then one context doc per source URL, with the vector store as a NumPy array and a single matmul; chose MMR over plain top-k because top-k returns five outlets reporting the same sentence and spends the window that the remaining required facts needed
* Treated embedding spend as a first-class cost: a word-overlap prefilter caps the corpus before anything is embedded, batches respect the API's per-call limit, and query and document text are embedded with different `task_type`s
* Eliminated a class of citation hallucination by reconstructing citations from the retrieved context using the source indices the model returns, so a cited URL is provably one the agent actually fetched
* Treated retrieved web text as untrusted input: every document is fenced in a delimiter, each field is length-capped to the chunk budget, and both retrieval-bearing prompts open by instructing the model that the fenced content is data and never commands, kept checkable by confining every prompt string to one module
* Hardened the agent against production failure modes: timeouts on all three network paths (SerpAPI's own default is 60,000 seconds) and bounded byte reads so a 40MB page cannot become a 40MB string, exponential-backoff retry on 429s and 5xx, deterministic safety blocks deliberately not retried, and rolling model aliases after two pinned models were retired mid-development
* Designed the degradation chain so quality falls before availability: no page text falls back to snippet, a down embedding API falls back to search-rank order, and no context at all returns an explicit fallback answer flagged `degraded` and exits non-zero; fixed the machine-readable output contract by routing all diagnostics to stderr so piping a failing run into a parser no longer crashes
* Built an evaluation harness over a 25-question golden set scoring citation validity, keyword recall, slot-fill rate, reflection-loop firing rate, answer-length compliance, LLM calls per question as a cost metric, and interpolated p95 latency; added `context_recall` to separate a search that never found the fact from a model handed it and dropping it, and `page_chunk_rate` to expose a silent collapse to snippet fallback
* Designed the golden set to grade refusal as well as recall: false-premise and unknowable questions fail on `must_not_include`, so a fluent, well-cited, schema-valid answer to an unanswerable question scores as a failure
* Wrote an 87-test Pytest suite asserting which graph nodes ran and what reached the prompt rather than only the final output, with all Gemini and SerpAPI calls mocked so the suite needs no API keys and completes in under a second
* Set up GitHub Actions CI running Ruff, the test suite, and a Docker image build on every push and pull request; made the project pip-installable with a console entry point, split test tooling out of the runtime requirements, added upper bounds to unpinned dependencies, and set the container to run as a non-root user
...
### Traffic Congestion Analysis from Live NSW Traffic Cameras

**Python, OpenCV, scikit-image, PyTorch, scikit-learn, NumPy, Flask, TfNSW Open Data API | github.com/BillVu05/traffic-congestion-cv | University of Technology Sydney | Sep 2025 – Aug 2026**

* Led a team of 6 on an image-processing and pattern-recognition pipeline turning public Transport for NSW traffic-camera snapshots into structured congestion signals; owned dataset collection, API integration, the preprocessing pipeline, the repository, and the evaluation harness
* Integrated the TfNSW Live Traffic Cameras API to collect 192 camera snapshots and a 194-camera GeoJSON manifest (GPS coordinates, view descriptions), with response validation, retries, and a mock API server for offline testing
* Built the preprocessing pipeline (non-local-means denoising, CLAHE on the LAB lightness channel, road-mask and night-enhancement variants), raising RMS contrast by 15.8% and mean brightness by 19.0% across the 108 low-light frames
* Found that all four detectors were counting the burned-in camera timestamp and watermark overlays as vehicles, and fixed it with a shared region-of-interest mask applied to every method and every image-quality statistic
* Built the evaluation harness scoring 4 vehicle detection methods (background subtraction, edge and blob analysis, template matching, adaptive thresholding) and the team's convolutional autoencoder against hand-annotated ground truth, with fixed dev/test splits, shared post-processing filters so only the detection principle varies, and three baselines
* Closed three data-leakage paths: templates mined from the dev split only, the autoencoder's anomaly threshold set on training error rather than on the frames being scored, and count calibration fitted on dev and applied unchanged to test
* Replaced point-estimate rankings with statistical comparison: 95% bootstrap confidence intervals (5,000 resamples), a Wilcoxon signed-rank test against the best baseline, and 5 success criteria fixed before scoring and checked automatically in code, with fixed seeds so every reported figure reproduces
* Reported the findings as measured: no detector beat an edge-density regression baseline (best MAE 2.13 vs 2.09, p = 1.0), CLAHE preprocessing improved every image-quality metric yet degraded detection for 3 of 4 methods, autoencoder anomaly detection scored at chance (ROC-AUC 0.455), and only the runtime target was met (3.9 ms/frame)
...
* **BI & Reporting:** Advanced Excel (Power Query, Pivot Tables, INDEX/MATCH, XLOOKUP, What-If Analysis, Solver), Power BI, Looker Studio, GA4, Google Search Console, Dashboard Design
